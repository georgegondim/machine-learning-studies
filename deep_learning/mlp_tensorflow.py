...
def inference(x):
    ...
    hidden_weights = tf.Variable(tf.random_normal(
        [input_size, hidden_size], stddev=0.3))
    hidden_bias = tf.Variable(tf.random_normal([hidden_size], stddev=0.3))
    output_weights = tf.Variable(tf.random_normal(
        [hidden_size, output_size], stddev=0.3))
    output_bias = tf.Variable(tf.random_normal(
        [output_size], stddev=0.3))

    # Feedforward
    hidden_activations = tf.matmul(x, hidden_weights) \
        + hidden_bias
    hidden_activations = tf.nn.sigmoid(hidden_activations)

    output_activations = tf.matmul(hidden_activations, output_weights) \
        + output_bias
    output_activations = tf.nn.softmax(output_activations)
...
def loss(outputs, y):
    # Cross-entropy loss
    cross_entropy = -tf.reduce_sum(tf.multiply(y, tf.log(outputs)), 1)
...
def get_corrects(outputs, y):
    ...
    return tf.reduce_sum(tf.cast(tf.equal(prediction, correct), tf.float32))


def evaluate(sess, loss, correct_count, x_placeholder, y_placeholder, x, y,
             optimizer=None):
    corrects = 0
    avg_loss = 0.0
    ...
    return avg_loss, corrects / x.shape[0]
